...
const { GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, LOUD } = process.env;
...
export default class AuthService {
  ...
  static async githubAuth(query) {
    try {
      ...
      if (LOUD === 'true') console.log('Accessing github user:', user);

      // check if user already in database
      const match = await User.findBy(user.id);
      if (LOUD === 'true') console.log('User in database:', match);
