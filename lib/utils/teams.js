...
const findTopGroups = data => {
  ...
  const prefs = data.preferences.map(pref => {
    const pitchList = shuffleArray(data.pitches);
    const prefList = [...pref];
    for (let pitch of pitchList) if (!prefList.includes(pitch)) prefList.push(pitch);
    return prefList;
  });
...
const literallyRandom = (preferences, pitches) => {
  ...
  preferences = preferences.map(pref => {
    const pitchList = shuffleArray(pitches);
    const prefList = [...pref];
    for (let pitch of pitchList) if (!prefList.includes(pitch)) prefList.push(pitch);
    return prefList;
  });
...
export default (data, opts = {}) => {
  ...
  const parsedData = options.raw ? {
    ...
    preferences: data.preferences
      .filter((preference, i) => i === data.preferences.findIndex(
